...
export async function sendInviteLinkEmail(p: InviteEmailParams): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  const subject = 'Your invitation to Travail'
  const text = [
    `Hi ${first},`,
    '',
  ...
  const html = brandedInviteEmail(p, first)
  ...
  try {
    ...
    await logInvite(p, 'sent', resendId, null)
    return { ok: true }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    await logInvite(p, 'failed', null, msg)
    return { ok: false, error: msg }
  }
}

async function logInvite(p: InviteEmailParams, status: 'sent' | 'failed', resendId: string | null, error: string | null): Promise<void> {
  ...
  try {
    ...
    await (admin as any).from('email_log').insert({
      ...
      to_email: p.to,
      subject: 'Your invitation to Travail',
      resend_id: resendId,
      refs: { join_url: p.joinUrl },
      send_status: status,
      send_error: error,
      bcc_ops: false,
    })
...
function brandedInviteEmail(p: InviteEmailParams, first: string): string {
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<meta name="color-scheme" content="light only"/><title>Your invitation to Travail</title></head>
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:580px;background:#fffbf0;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(13,51,64,0.10);">
        <tr><td style="background:linear-gradient(135deg,#042128 0%,#0a3340 52%,#073744 100%);padding:34px 36px 30px;color:#fff;">
          <div style="font-family:'JetBrains Mono','Courier New',monospace;font-size:10px;letter-spacing:0.22em;text-transform:uppercase;color:#00b3c7;font-weight:700;margin-bottom:10px;">By private invitation</div>
          <div style="font-size:32px;font-weight:700;color:#fff;letter-spacing:-0.022em;line-height:1.05;font-family:'Inter Tight',sans-serif;">Welcome to Travail.</div>
        </td></tr>
        ...
        <tr><td style="padding:8px 36px 18px;">
          <div style="font-size:14px;color:#1f4856;line-height:1.65;">You&rsquo;ve been invited to Travail &mdash; private aviation and curated experiences with a small, vetted network. Tap below to set up your account; it takes about a minute.</div>
        </td></tr>
        <tr><td style="padding:4px 36px 22px;">
          <a href="${esc(p.joinUrl)}" target="_blank" rel="noopener" style="display:inline-block;padding:14px 26px;background:#00b3c7;color:#fff;text-decoration:none;border-radius:10px;font-size:15px;font-weight:700;font-family:'Inter Tight',sans-serif;letter-spacing:-0.005em;">Set up your account &rarr;</a>
        </td></tr>
        ...
        <tr><td style="padding:14px 36px 24px;border-top:1px solid rgba(13,51,64,0.08);">
          <div style="font-size:12px;color:#6b7c80;line-height:1.55;">This invitation is just for you and expires in 30 days. Questions? Reply to this email and Ops will follow up.</div>
        </td></tr>
      </table>
    </td></tr>
  </table>
