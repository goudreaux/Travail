...
const SECTIONS = [
  {
    ...
    items: [
      ...
      'PWA-ready, fully mobile responsive — purpose-built layouts for ≤768px and ≤360px',
    ],
  },
  {
    title: 'Ops back-of-house',
    color: 'sun',
    items: [
      'Review queue — anchor submissions, cancellation requests, waitlist, and proposals in one place',
      'Anchor quote workflow — set price, send quote, member accepts, ops publishes, card captures',
      'Trip Proposals review — approve with capacity + min seats + price, decline with reason, lock with one click',
      'Atomic proposal lock with idempotency guard — no double-charges from double-clicks',
  ...
  },
  {
    ...
    items: [
      ...
      'Force-cancel refund path — only path that refunds the 3% service fee (Travail eats it)',
      'Idempotent capture with per-commit failure isolation — one decline doesn’t kill the trip',
  ...
  },
  {
    ...
    color: 'tropic',
    items: [
      'Refer-a-friend system — member submits → ops reviews → invite-as-member workflow',
  ...
  },
  {
    ...
    color: 'sun',
    items: [
      'BCC ops on every member email for paper trail (override via OPS_PAPER_TRAIL_BCC)',
      ...
      'Ops notification emails for every billable event (charge, refund, dispute, signup, payment failure)',
