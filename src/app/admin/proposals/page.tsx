  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--hair)', borderRadius: 12, padding: '18px 20px' }}>
      ...
      {row.status === 'open' && (
        <>
          {(() => {
            const committed = row.network_seats + (row.proposer_min_seats ?? 0)
            // Fundable when committed seats + the proposer's guarantee can reach
            // the minimum — same rule the lock route enforces.
            const canLock = row.min_seats != null && (row.network_seats + (row.proposer_max_seats ?? 0)) >= row.min_seats
            return (
              <>
                <div style={{ marginBottom: 10, fontSize: 13, color: 'var(--ink-soft)' }}>
                  <strong style={{ color: 'var(--ink)' }}>{committed}</strong>/{row.min_seats ?? '?'} minimum commits
                ...
                <button
                  ...
                  onClick={onLock}
                  title={canLock ? '' : 'Waiting on enough commits to reach the minimum'}
                >
                  {busy ? 'Locking…' : canLock ? 'Lock & capture all commits' : 'Waiting on commits'}
                </button>
              </>
            )
          })()}
